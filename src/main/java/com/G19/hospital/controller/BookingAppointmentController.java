...
//     // }
//         @GetMapping
//     public ResponseEntity<Page<BookingAppointment>> getAllBookingAppointments(
//             @RequestParam(defaultValue = "0") int page,
//             @RequestParam(defaultValue = "10") int size) {

//         PageRequest pageReq = PageRequest.of(page, size);
//         Page<BookingAppointment> bookingAppointments =
//             bookingAppointmentServices.getAllBookingAppointments(pageReq);
...
public class BookingAppointmentController {
    ...
    // … create/update/cancel/completed endpoints unchanged …
    @PostMapping
    public ResponseEntity<BookingAppointment> createBookingAppointment(
            @RequestBody BookingAppointmentDTO bookingAppointmentDTO) {
        try {
    ...
    @PostMapping("/byStaff")
    public ResponseEntity<BookingAppointment> createBookingAppointmentByStaff(
            @RequestBody BookingAppointmentDTO bookingAppointmentDTO) {
        try {
            BookingAppointment createdBookingAppointment = bookingAppointmentServices
                    .createBookingAppointment(bookingAppointmentDTO);
    ...
    public ResponseEntity<Page<BookingAppointment>> getAllBookingAppointments(
        ...
        Page<BookingAppointment> p = bookingAppointmentServices
                .getAllBookingAppointments(PageRequest.of(page, size));
    ...
    public ResponseEntity<BookingAppointment> getBookingAppointmentById(@PathVariable Long id) {
        try {
            return ResponseEntity.ok(bookingAppointmentServices.getBookingAppointmentById(id));
        } catch (Exception e) {
            return ResponseEntity.notFound().build();
        }
    }

    // // BY DOCTOR (plain list)
    // @GetMapping("/doctor/{doctorId}")
    // public ResponseEntity<List<BookingAppointment>> getBookingsByDoctorId(@PathVariable Long doctorId) {
    //     User doctor = new User();
    //     doctor.setId(doctorId);
    //     return ResponseEntity.ok(bookingAppointmentServices.getBookingsByDoctorId(doctor));
    // }

    // BY DOCTOR (paged)
    @GetMapping(value = "/doctor/{doctorId}")
    public ResponseEntity<Page<BookingAppointment>> getBookingsByDoctorIdPaged(
        ...
        doctor.setId(doctorId);
        Page<BookingAppointment> p = bookingAppointmentServices
                .getBookingsByDoctorId(doctor, PageRequest.of(page, size));
        return ResponseEntity.ok(p);
    }

    // MY APPTS FOR DOCTOR (plain)
    // @GetMapping("/doctor/my-appointments")
    // public ResponseEntity<List<BookingAppointment>> getMyAppointments() {
    //     try {
    //         Authentication auth = SecurityContextHolder.getContext().getAuthentication();
    //         String user = auth.getName();
    //         User doctor = userRepository.findByUsername(user)
    //                 .orElseThrow(() -> new RuntimeException("Doctor not found"));
    ...
    // MY APPTS FOR DOCTOR (paged)
    @GetMapping(value = "/doctor/my-appointments")
    public ResponseEntity<Page<BookingAppointment>> getMyAppointmentsPaged(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        try {
            Authentication auth = SecurityContextHolder.getContext().getAuthentication();
    ...
    //     User patient = new User();
    //     patient.setId(patientId);
    //     return ResponseEntity.ok(bookingAppointmentServices.getBookingsByPatientId(patient));
    // }

    // BY PATIENT (paged)
    @GetMapping(value = "/patient/{patientId}")
    public ResponseEntity<Page<BookingAppointment>> getBookingsByPatientIdPaged(
        ...
        patient.setId(patientId);
        Page<BookingAppointment> p = bookingAppointmentServices
                .getBookingsByPatientId(patient, PageRequest.of(page, size));
        return ResponseEntity.ok(p);
    }

    // MY APPTS FOR PATIENT (plain)
    // @GetMapping("/patient/my-appointments")
    // public ResponseEntity<List<BookingAppointment>> getPatientAppointments() {
    //     try {
    //         Authentication auth = SecurityContextHolder.getContext().getAuthentication();
    //         String user = auth.getName();
    //         User patient = userRepository.findByUsername(user)
    //                 .orElseThrow(() -> new RuntimeException("Patient not found"));
    //         return ResponseEntity.ok(bookingAppointmentServices.getBookingsByPatientId(patient));
    //     } catch (Exception e) {
    //         return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    // MY APPTS FOR PATIENT (paged)
    @GetMapping(value = "/patient/my-appointments")
    public ResponseEntity<Page<BookingAppointment>> getPatientAppointmentsPaged(
                 @RequestParam(defaultValue = "0") int page,
        @RequestParam(defaultValue = "10") int size) {
        try {
            Authentication auth = SecurityContextHolder.getContext().getAuthentication();
    ...
    //     return ResponseEntity.ok(bookingAppointmentServices.getBookingsByScheduleId(sched));
    // }

    // BY SCHEDULE (paged)
    @GetMapping(value = "/schedule/{scheduleId}")
    public ResponseEntity<Page<BookingAppointment>> getBookingsByScheduleIdPaged(
        ...
        sched.setScheduleId(scheduleId);
        Page<BookingAppointment> p = bookingAppointmentServices
                .getBookingsByScheduleId(sched, PageRequest.of(page, size));
        return ResponseEntity.ok(p);
    }

    // TOKEN & COUNT endpoints unchanged…
    @GetMapping("/token/{token}")
    public ResponseEntity<BookingAppointment> getBookingByToken(@PathVariable String token) {
        Optional<BookingAppointment> booking = bookingAppointmentServices.getBookingByToken(token);
        return booking.map(ResponseEntity::ok)
