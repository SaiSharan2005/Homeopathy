...
public class DoctorServicesImplement implements DoctorServices {
    ...
    @Override
    public DoctorRegister registerDoctor(DoctorRegister doctorRegister) throws Exception {
        if (doctorRepository.findByPhoneNumber(doctorRegister.getPhoneNumber()) != null) {
            throw new Exception("Phone number already in use");
        }
    ...
    @Override
    public DoctorDetails profileDoctor(DoctorDetailsDTO doctorDetailsDTO) throws Exception {
        DoctorRegister doctorRegister = doctorRepository.findById(doctorDetailsDTO.getDoctorId())
